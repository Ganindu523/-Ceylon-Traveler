...
const BookingPage = () => {
    ...
    const fetchBookings = useCallback(async () => {
        try {
            ...
            // NOTE: Auto-select removed as per request
        } catch (err) {
    ...
    const calculateCosts = (trip) => {
        ...
        (trip?.destinations || []).forEach((d) => {
            if (d?.selectedRooms && d.selectedRooms.length > 0) {
                ...
                items.push({ 
                    label: `Hotel: ${d.hotelDetails?.name || d.district || 'N/A'}`,
                    subLabel: `${d?.nights || 0} Nights`,
                    priceUSD: legTotalUSD
                });
        ...
        (trip?.destinations || []).forEach((d, i) => {
            if (d?.selectedGuideDetails) {
                ...
                items.push({ 
                    label: `Guide: ${d.selectedGuideDetails?.name || 'N/A'}`,
                    subLabel: `Leg ${i+1}`,
                    priceUSD: guideTotalUSD
                });
            ...
            <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-4 gap-8">
                ...
                <div className="lg:col-span-1 h-[85vh] flex flex-col bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
                    <div className="p-5 border-b border-gray-100 bg-gray-50/50">
                        <h2 className="text-xl font-black text-gray-800 flex items-center gap-2"><Clock size={20} className="text-teal-600"/> History</h2>
                    </div>
                    ...
                    <div className="flex-1 overflow-y-auto p-3 custom-scrollbar space-y-3">
                        {bookings.length === 0 && (
                            <div className="text-center py-10">
                                ...
                                <button onClick={() => navigate('/plan-trip')} className="mt-2 text-xs font-bold text-teal-600 hover:underline">Plan a Trip</button>
                            </div>
                        )}
                        ...
                        {bookings.map(b => (
                            <div 
                                ...
                                className={`p-4 rounded-2xl border cursor-pointer transition-all hover:shadow-md ${selectedTrip?._id === b._id ? 'bg-teal-50 border-teal-500 ring-1 ring-teal-500' : 'bg-white border-gray-100 hover:border-teal-300'}`}
                            >
                                <div className="flex justify-between items-center mb-1">
                                    <span className="font-bold text-sm">#{b._id.slice(-6).toUpperCase()}</span>
                                ...
                                <div className="text-xs opacity-80 flex items-center gap-1 mt-2">
                                    <Calendar size={12}/> {b.createdAt ? new Date(b.createdAt).toLocaleDateString() : 'N/A'}
                                </div>
                                <div className="text-xs font-bold mt-1 opacity-90">
                                    {(b?.destinations || []).length} Stops • {b?.destinations?.[0]?.district || 'TBD'}
                                </div>
                            </div>
                        ))}
                    </div>
                            ...
                            <div className="flex flex-col md:flex-row justify-between items-center mb-6 bg-white p-6 rounded-3xl shadow-sm border border-gray-100">
                                ...
                                <div className="flex gap-3 mt-4 md:mt-0">
                                    {selectedTrip.status === 'Pending' && (
                                        <>
                                            <button onClick={handleCancelTrip} className="px-4 py-2 bg-red-50 text-red-600 font-bold rounded-xl text-sm hover:bg-red-100 transition border border-red-100">Cancel Booking</button>
                                            <button onClick={() => navigate('/plan-trip')} className="px-4 py-2 bg-gray-100 text-gray-700 font-bold rounded-xl text-sm hover:bg-gray-200 transition flex items-center gap-2 border border-gray-200"><Edit3 size={14}/> Edit Plan</button>
                                        </>
                                    )}
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
                                ...
                                <div className="xl:col-span-2 space-y-6">
                                    {(selectedTrip?.destinations || []).map((dest, i) => (
                                        <div key={i} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 relative overflow-hidden group hover:shadow-md transition-shadow">
                                            <div className="absolute left-0 top-0 bottom-0 w-1.5 bg-teal-500"></div>
                                            <div className="flex justify-between items-start mb-4">
                                                <h3 className="text-lg font-black text-gray-800 flex items-center gap-2"><MapPin className="text-teal-500" size={20}/> {dest?.district || 'N/A'}</h3>
                                                <p className="text-xs text-gray-400 font-bold uppercase tracking-wider bg-gray-100 px-2 py-1 rounded">Leg {i+1}</p>
                                            </div>
                                                ...
                                                <div className="bg-gray-50 p-3 rounded-xl border border-gray-200">
                                                    ...
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        <div className="overflow-hidden">
                                                            <p className="font-bold text-sm text-gray-800 truncate">{dest?.hotelDetails?.name || 'None'}</p>
                                                            <p className="text-[10px] text-gray-500">{dest?.nights || 0} Nights</p>
                                                        </div>
                                                    </div>
                                                </div>
                                                <div className="bg-gray-50 p-3 rounded-xl border border-gray-200">
                                                    ...
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        <div>
                                                            <p className="font-bold text-sm text-gray-800">{dest?.transportCost ? 'Taxi' : 'None'}</p>
                                                            <p className="text-[10px] text-gray-500">{dest?.distanceKm || 0} KM</p>
                                                        </div>
                                                    </div>
                                                </div>
                                                <div className="bg-gray-50 p-3 rounded-xl border border-gray-200">
                                                    ...
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        <div className="overflow-hidden">
                                                            <p className="font-bold text-sm text-gray-800 truncate">{dest?.selectedGuideDetails?.name || 'None'}</p>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                ...
                                <div className="xl:col-span-1">
                                    <div className="bg-white rounded-3xl shadow-xl border border-gray-100 p-6 sticky top-6">
                                                    ...
                                                    <div className="text-right">
                                                        {item?.priceUSD !== undefined ? (
                                                            <>
                                                                ...
                                                                <p className="text-[10px] text-gray-400">≈ LKR {((item.priceUSD || 0) * USD_LKR).toLocaleString()}</p>
                                                            </>
                                                        ) : (
                                        ...
                                        <div className="bg-gray-900 rounded-xl p-5 mb-6 text-white shadow-lg shadow-gray-200">
                                        ...
                                        {selectedTrip?.status === 'Pending' ? (
                                            <button onClick={() => setShowPaymentModal(true)} className="w-full py-4 bg-teal-600 hover:bg-teal-700 text-white rounded-xl font-bold flex justify-center items-center gap-2 transition shadow-lg shadow-teal-200">
                                                <CreditCard size={18}/> Pay & Confirm
                                            </button>
                                        ) : selectedTrip?.status === 'Confirmed' ? (
                                            <div className="bg-green-50 border border-green-200 rounded-xl p-4 text-center">
                                                <CheckCircle className="mx-auto text-green-600 mb-2" size={32}/>
                                                <p className="font-bold text-green-800">Paid & Confirmed</p>
                                                <p className="text-xs text-green-600 mt-1">Paid on {selectedTrip?.paymentDetails?.paidAt ? new Date(selectedTrip.paymentDetails.paidAt).toLocaleDateString() : 'N/A'}</p>
                                            </div>
                                        ) : (
                                            <div className="bg-red-50 border border-red-200 rounded-xl p-4 text-center text-red-600 font-bold">
                                                Trip Cancelled
                                            </div>
                                        )}
                                    </div>
                        ...
                        <form onSubmit={handlePaymentSubmit} className="p-8 space-y-5">
                            <div>
                                ...
                                <input required type="text" placeholder="0000 0000 0000 0000" maxLength="19"
                                    value={paymentForm.cardNumber} onChange={(e) => setPaymentForm({...paymentForm, cardNumber: e.target.value})}
                                    className="w-full border-2 border-gray-200 rounded-xl px-4 py-2.5 font-mono text-gray-700 outline-none"/>
                            </div>
                            <div className="grid grid-cols-2 gap-4">
                                <div><label className="text-xs font-bold text-gray-500 uppercase mb-1 block">Expiry</label><input required type="text" placeholder="MM/YY" className="w-full border-2 border-gray-200 rounded-xl px-4 py-2.5 outline-none"/></div>
                                <div><label className="text-xs font-bold text-gray-500 uppercase mb-1 block">CVV</label><input required type="password" placeholder="123" className="w-full border-2 border-gray-200 rounded-xl px-4 py-2.5 outline-none"/></div>
                            </div>
                            <div>
                                <label className="text-xs font-bold text-gray-500 uppercase mb-1 block">Name</label>
                                <input required type="text" placeholder="Cardholder Name" value={paymentForm.cardName} onChange={(e) => setPaymentForm({...paymentForm, cardName: e.target.value})} className="w-full border-2 border-gray-200 rounded-xl px-4 py-2.5 outline-none"/>
                            </div>
                            <button type="submit" disabled={processingPayment} className="w-full py-4 bg-teal-600 text-white font-bold rounded-xl shadow-lg hover:bg-teal-700 disabled:opacity-70 flex justify-center items-center gap-2">
                                {processingPayment ? <Loader className="animate-spin"/> : `Pay LKR ${(grandTotalLKR || 0).toLocaleString()}`}
                            </button>
                        </form>
                    </div>
                </div>
